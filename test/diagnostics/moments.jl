@testset "Spectral diagnostics" begin
    ...
    @test all(mxx .+ myy .≈ pi / 2 * (cgrid.kappa_faces[end]^4 - cgrid.kappa_faces[1]^4))
    expected_mean_square = (pi / 2 * (cgrid.kappa_faces[end]^4 - cgrid.kappa_faces[1]^4)) /
                           (pi * (cgrid.kappa_faces[end]^2 - cgrid.kappa_faces[1]^2))
...
@testset "Frequency-direction diagnostics" begin
    ...
    theta = range(0, 2pi; length=9)[1:8]
    cgrid = FrequencyDirectionGrid(Float64; frequency=range(0.08, 0.2; length=4), theta=theta)
    N = WaveActionField(grid, cgrid)
    set!(N, 1.0)
    ...
    trace = pi / 2 * (cgrid.kappa_faces[end]^4 - cgrid.kappa_faces[1]^4)
    ...
    @test peak_wavenumber(N)[1, 1] == cgrid.kappa[3]
    @test deep_water_peak_phase_speed(N; gravity=9.81)[1, 1] ≈ sqrt(9.81 / cgrid.kappa[3])
    @test wave_age(N, fill(2.0, 1, 1); gravity=9.81)[1, 1] ≈ sqrt(9.81 / cgrid.kappa[3]) / 2
    ...
    pgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=theta)
