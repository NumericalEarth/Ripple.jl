@testset "Source terms" begin
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=relaxation, timestepper=:ForwardEuler)
    set!(model, N=0.0)
    time_step!(model, 0.5)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=balanced, timestepper=:ForwardEuler)
    set!(model, N=1.5)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) < 1e-14

    damping = BottomFriction(rate=100.0)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=damping, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    time_step!(model, 1.0)
...
@testset "Directional wind and saturation sources" begin
    grid = RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1))
    cgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0, pi/2, pi, 3pi/2])

    wind = ExponentialWindInput(rate=0.5, direction=0.0, spreading_power=2)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=wind, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)

    wind_weights = [Ripple.wind_directional_weight(cgrid, 1, n, 0.0, 2)
                    for n in eachindex(cgrid.theta)]
    ...
    bad_model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=invalid_spreading)
    set!(bad_model, N=1.0)
...
@testset "Power-law wind input" begin
    ...
    storm_wind = PowerLawWindInput(rate=0.25,
                                   ...
                                   speed_power=1.0,
                                   spreading_power=1.0)
    storm_model = SpectralWaveModel(; grid=storm_grid,
                                      spectral_grid=cgrid,
                                      sources=storm_wind,
    ...
    negative = PowerLawWindInput(rate=-0.5,
                                 ...
                                 speed_power=1.0,
                                 spreading_power=1.0)
    damping_model = SpectralWaveModel(; grid,
                                        spectral_grid=cgrid,
                                        sources=negative,
    ...
    bad_model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=invalid)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    invalid_reference = PowerLawWindInput(rate=0.1, reference_speed=0.0)
    bad_model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=invalid_reference)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Wave-age wind input" begin
    grid = RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1))
    cgrid = PolarWaveVectorGrid(Float64; kappa=[0.25, 1.0], theta=[0.0, pi])
    ...
    opposing = WaveAgeWindInput(rate=0.5,
                                ...
                                spreading_power=0.0,
                                gravity=1.0)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=opposing)
    set!(model, N=3.0)
    compute_tendencies!(model)
    ...
    negative = WaveAgeWindInput(rate=-0.5,
                                ...
                                spreading_power=0.0,
                                gravity=1.0)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=negative, timestepper=:SemiImplicitEuler)
    set!(model, N=3.0)
...
@testset "Time-dependent wind source parameters" begin
    grid = RectilinearGrid(CPU(); size=(2, 1, 1), x=(0, 2), y=(0, 1), z=(0, 1))
    cgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0, pi/2])
...
@testset "Whitecapping dissipation" begin
    grid = RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1))
    cgrid = PolarWaveVectorGrid(Float64; kappa=[0.5, 1.0, 2.0], theta=[0.0])
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=whitecapping, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)

    spectral_factor = [spectral_radial_power_average(cgrid, m, 1, 2)
                       for m in eachindex(cgrid.kappa)]
    @test model.tendencies[1, 1, 1, 1] < 0
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=weak, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) < 1e-14
end
...
@testset "Frequency dissipation" begin
    grid = RectilinearGrid(CPU(); size=(2, 1, 1), x=(0, 2), y=(0, 1), z=(0, 1))
    theta = range(0, 2pi; length=5)[1:4]
    cgrid = FrequencyDirectionGrid(Float64; frequency=[0.1, 0.2, 0.4], theta=theta)
    ...
    for n in eachindex(cgrid.theta), m in eachindex(cgrid.frequency)
    ...
    pgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0])
...
@testset "Wavenumber dissipation" begin
    ...
    initial = copy(interior(damping_model.action))
    time_step!(damping_model, 0.25)
    for n in eachindex(cgrid.theta), m in eachindex(cgrid.kappa)
...
@testset "Mean-frequency dissipation" begin
    ...
    model = SpectralWaveModel(; grid,
                                ...
                                timestepper=:ForwardEuler)
    set!(model, N=(x, y, kx, ky) -> abs(hypot(kx, ky) - cgrid.kappa[3]) < 1e-12 ? 2.0 : 0.5)
    ...
    pgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0])
    model = SpectralWaveModel(; grid, spectral_grid=pgrid, sources=MeanFrequencyDissipation(rate=0.1))
    set!(model, N=1.0)
...
@testset "Peak-frequency dissipation" begin
    ...
    model = SpectralWaveModel(; grid,
                                ...
                                sources=source,
                                timestepper=:ForwardEuler)
    set!(model, N=(x, y, kx, ky) -> abs(hypot(kx, ky) - cgrid.kappa[3]) < 1e-12 ? 4.0 : 1.0)
    ...
    pgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0])
    bad_model = SpectralWaveModel(; grid, spectral_grid=pgrid, sources=source)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
...
@testset "Mean-square-wavenumber dissipation" begin
    ...
    bad_model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=invalid_power)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Peak-wavenumber dissipation" begin
    grid = RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1))
    cgrid = PolarWaveVectorGrid(Float64; kappa=[0.5, 1.0, 2.0], theta=[0.0, pi])
...
@testset "Depth-limited breaking" begin
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=breaking, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=stiff, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=invalid)
    set!(model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(model)
end
...
@testset "Depth-dependent bottom friction" begin
    grid = RectilinearGrid(CPU(); size=(2, 1, 1), x=(0, 2), y=(0, 1), z=(0, 1))
    cgrid = PolarWaveVectorGrid(Float64; kappa=[0.5, 1.0], theta=[0.0])
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=friction, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=disabled, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) < 1e-14
...
@testset "Ice and swell damping" begin
    ...
    ice = IceDamping(rate=0.2,
                     ...
                     wavenumber_power=2.0,
                     reference_wavenumber=0.5)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=ice, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    cgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0, pi/2, pi])
    swell = SwellDissipation(rate=0.3, direction=0.0, spreading_power=2)
    model = SpectralWaveModel(; grid=RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1)),
                                spectral_grid=cgrid,
                                sources=swell,
                                timestepper=:ForwardEuler)
    set!(model, N=1.0)
...
@testset "Mean-direction dissipation" begin
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=source, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=source, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
    model.action[1, 1, 1, 1] = 10.0
    time_step!(model, 1.0)
    ...
    isotropic = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=source)
    set!(isotropic, N=1.0)
...
@testset "Directional diffusion source" begin
    grid = RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1))
    theta = range(0, 2pi; length=9)[1:8]
    cgrid = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=theta)
    diffusion = DirectionalDiffusion(rate=0.1)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=diffusion, timestepper=:ForwardEuler)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=diffusion, timestepper=:SemiImplicitEuler)
    set!(model, N=(x, y, kx, ky) -> ky > 0 && abs(kx) < 1e-12 ? 10.0 : 1.0)
    time_step!(model, 0.1)
    @test all(interior(model.action) .>= 0)
    ...
    bad_model = SpectralWaveModel(; grid, spectral_grid=cartesian, sources=diffusion)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    negative = DirectionalDiffusion(rate=-0.1)
    bad_model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=negative)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Directional advection source" begin
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=advection, timestepper=:ForwardEuler)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=clockwise, timestepper=:ForwardEuler)
    set!(model, N=(x, y, kx, ky) -> ky < 0 && abs(kx) < 1e-12 ? 10.0 : 1.0)
    compute_tendencies!(model)
    weighted_tendency = sum(model.tendencies[1, 1, 1, n] * spectral_weight(cgrid, 1, n)
                            for n in eachindex(cgrid.theta))
    ...
    fgrid = FrequencyDirectionGrid(Float64; frequency=[0.1, 0.2], theta=theta)
    ...
    weighted_tendency = sum(model.tendencies[1, 1, m, n] * spectral_weight(fgrid, m, n)
                            for n in eachindex(fgrid.theta), m in eachindex(fgrid.frequency))
    @test abs(weighted_tendency) < 1e-12

    one_direction = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0])
    model = SpectralWaveModel(; grid, spectral_grid=one_direction, sources=advection)
    set!(model, N=1.0)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) == 0

    cartesian = CartesianWaveVectorGrid(Float64; kx=[0.0], ky=[0.0])
    bad_model = SpectralWaveModel(; grid, spectral_grid=cartesian, sources=advection)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    bounded_direction = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0, pi],
                                            topology=(NoFlux(), Bounded()))
    bad_model = SpectralWaveModel(; grid, spectral_grid=bounded_direction, sources=advection)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Radial diffusion source" begin
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=RadialDiffusion(rate=0.05), timestepper=:SemiImplicitEuler)
    set!(model, N=(x, y, kx, ky) -> abs(hypot(kx, ky) - 1.0) < 1e-12 ? 10.0 : 1.0)
    time_step!(model, 0.1)
    @test all(interior(model.action) .>= 0)
    ...
    periodic_radial = PolarWaveVectorGrid(Float64; kappa=[0.5, 1.0], theta=[0.0],
                                          topology=(Periodic(), Periodic()))
    bad_model = SpectralWaveModel(; grid, spectral_grid=periodic_radial, sources=diffusion)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
...
@testset "Radial advection source" begin
    ...
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=downshift, timestepper=:ForwardEuler)
    ...
    fgrid = FrequencyDirectionGrid(Float64; frequency=[0.1, 0.2, 0.4], theta=[0.0])
    ...
    periodic_radial = PolarWaveVectorGrid(Float64; kappa=[0.5, 1.0], theta=[0.0],
                                          topology=(Periodic(), Periodic()))
    bad_model = SpectralWaveModel(; grid, spectral_grid=periodic_radial, sources=advection)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Nonlinear spectral transfer source" begin
    ...
    bad_model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=negative_rate)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Nonlinear invariant transfer source" begin
    ...
    mx_tendency = sum(model.tendencies[1, 1, 1, n] * spectral_weight(cgrid, 1, n) *
                      k_components(cgrid, 1, n)[1] for n in eachindex(cgrid.theta))
    my_tendency = sum(model.tendencies[1, 1, 1, n] * spectral_weight(cgrid, 1, n) *
                      k_components(cgrid, 1, n)[2] for n in eachindex(cgrid.theta))
...
@testset "Triad spectral transfer source" begin
    grid = RectilinearGrid(CPU(); size=(1, 1, 1), x=(0, 1), y=(0, 1), z=(0, 1))
    cgrid = FrequencyDirectionGrid(Float64; frequency=[0.1, 0.2, 0.3], theta=[0.0])
    ...
    polar = PolarWaveVectorGrid(Float64; kappa=[1.0, 2.0], theta=[0.0])
    bad_model = SpectralWaveModel(; grid, spectral_grid=polar, sources=triad)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Discrete interaction approximation source" begin
    ...
    initial_energy = sum(model.action[1, 1, 1, n] * spectral_weight(cgrid, 1, n) *
                         cgrid.frequency[1] for n in eachindex(cgrid.theta))
    initial_mx, initial_my = first_moment(model.action)
    ...
    polar = PolarWaveVectorGrid(Float64; kappa=[1.0], theta=[0.0, pi/2, pi, 3pi/2])
    bad_model = SpectralWaveModel(; grid, spectral_grid=polar, sources=dia)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Source positive/damping split" begin
    ...
    for source in sources
        spectral_grid = source isa Union{FrequencyDissipation, MeanFrequencyDissipation, PeakFrequencyDissipation, TriadSpectralTransfer} ?
            FrequencyDirectionGrid(Float64; frequency=[0.1], theta=[0.0]) : cgrid
        spectral_grid = source isa TriadSpectralTransfer ?
            FrequencyDirectionGrid(Float64; frequency=[0.1, 0.2], theta=[0.0]) : spectral_grid
        spectral_grid = source isa DiscreteInteractionApproximation ?
            FrequencyDirectionGrid(Float64; frequency=[0.1], theta=[0.0, pi/2, pi, 3pi/2]) : spectral_grid
...
@testset "Semi-implicit source update" begin
    ...
    damping = BottomFriction(rate=100.0)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=damping, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
    time_step!(model, 1.0)
    @test model.action[1, 1, 1, 1] ≈ 1 / 101

    relaxation = RelaxationToSpectrum((x, y, kx, ky) -> 2.0; timescale=1.0)
    model = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=relaxation, timestepper=:SemiImplicitEuler)
    set!(model, N=0.0)
    time_step!(model, 0.5)
    ...
    model = SpectralWaveModel(; grid, spectral_grid=pgrid, sources=whitecapping, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
