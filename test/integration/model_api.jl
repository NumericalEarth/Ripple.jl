...
@testset "Model API" begin
    ...
    @test_throws ArgumentError SpectralWaveModel(; grid=:grid, spectral_grid=cgrid)
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=:spectral_grid)

    model = SpectralWaveModel(; grid, spectral_grid=cgrid, timestepper=:RK3)
    ...
    writer_model = SpectralWaveModel(grid; spectral_grid=cgrid)
    ...
    supplied_model = SpectralWaveModel(; grid, spectral_grid=cgrid, action=supplied_action)
    ...
    @test SpectralWaveModel(; grid, spectral_grid=cgrid, action=equivalent_action).action === equivalent_action
    ...
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, action=mismatched_physical_action)
    ...
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, action=mismatched_z_action)
    ...
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, action=mismatched_spectral_action)
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, action=zeros(4, 3, 3, 3))

    for timestepper in (:ForwardEuler, :SemiImplicitEuler, :AB2, :RK3, :LowStorageRK3, :LSRK3)
        @test SpectralWaveModel(; grid, spectral_grid=cgrid, timestepper).timestepper === timestepper
    end
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, timestepper=:RK4)
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, timestepper="RK3")
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, clock=:clock)
end
...
@testset "Oceananigans-style absent component semantics" begin
    ...
    empty_sources = SpectralWaveModel(; grid,
                                       spectral_grid=cgrid,
                                       sources=SourceTermSet())
    @test empty_sources.sources === nothing

    compatibility = SpectralWaveModel(; grid,
                                       spectral_grid=cgrid,
                                       sources=NoSource(),
    ...
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, sources=:wind)
    @test_throws ArgumentError SpectralWaveModel(; grid, spectral_grid=cgrid, sources=SourceTermSet((:wind,)))

    frequency_grid = FrequencyDirectionGrid(Float64; frequency=[0.1], theta=[0.0])
...
@testset "Source-only timestepper semantics" begin
    ...
    for n in eachindex(cgrid.theta), m in eachindex(cgrid.frequency), i in 1:2
...
@testset "AB2 and low-storage RK3 source-only timesteppers" begin
    ...
    rk3 = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=BottomFriction(rate=1.0), timestepper=:RK3)
    low_storage = SpectralWaveModel(; grid, spectral_grid=cgrid, sources=BottomFriction(rate=1.0), timestepper=:LowStorageRK3)
    set!(rk3, N=1.0)
...
@testset "External physical grid adaptation" begin
    ...
    model = SpectralWaveModel(; grid=MockExternalPhysicalGrid(), spectral_grid=cgrid)
