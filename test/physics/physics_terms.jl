@testset "Source terms" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=relaxation, timestepper=:ForwardEuler)
    set!(model, N=0.0)
    time_step!(model, 0.5)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=balanced, timestepper=:ForwardEuler)
    set!(model, N=1.5)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) < 1e-14

    damping = BottomFriction(rate=100.0)
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=damping, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    time_step!(model, 1.0)
...
@testset "Directional wind and saturation sources" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=wind, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=invalid_spreading)
    set!(bad_model, N=1.0)
...
@testset "Power-law wind input" begin
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=invalid)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    invalid_reference = PowerLawWindInput(rate=0.1, reference_speed=0.0)
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=invalid_reference)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Wave-age wind input" begin
    ...
    opposing = WaveAgeWindInput(rate=0.5,
                                ...
                                spreading_power=0.0,
                                gravity=1.0)
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=opposing)
    set!(model, N=3.0)
    compute_tendencies!(model)
    ...
    negative = WaveAgeWindInput(rate=-0.5,
                                ...
                                spreading_power=0.0,
                                gravity=1.0)
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=negative, timestepper=:SemiImplicitEuler)
    set!(model, N=3.0)
...
@testset "Whitecapping dissipation" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=whitecapping, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=weak, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) < 1e-14
end
...
@testset "Mean-frequency dissipation" begin
    ...
    pgrid = PolarWaveVectorGrid(; κ=[1.0], φ=[0.0])
    model = SpectralWaveModel(grid, pgrid; horizontal_advection=nothing, physics=MeanFrequencyDissipation(rate=0.1))
    set!(model, N=1.0)
...
@testset "Peak-frequency dissipation" begin
    ...
    pgrid = PolarWaveVectorGrid(; κ=[1.0], φ=[0.0])
    bad_model = SpectralWaveModel(grid, pgrid; horizontal_advection=nothing, physics=source)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    invalid_reference = PeakFrequencyDissipation(rate=0.1, reference_frequency=0.0)
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=no_advection, physics=invalid_reference)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    invalid_power = PeakFrequencyDissipation(rate=0.1, power=-1)
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=no_advection, physics=invalid_power)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Mean-square-wavenumber dissipation" begin
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=invalid_power)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Depth-limited breaking" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=breaking, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=stiff, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=invalid)
    set!(model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(model)
end
...
@testset "Depth-dependent bottom friction" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=friction, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=disabled, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) < 1e-14
...
@testset "Ice and swell damping" begin
    ...
    ice = IceDamping(rate=0.2,
                     ...
                     wavenumber_power=2.0,
                     reference_wavenumber=0.5)
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=ice, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    compute_tendencies!(model)
...
@testset "Mean-direction dissipation" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=source, timestepper=:ForwardEuler)
    set!(model, N=1.0)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=source, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
    model.action[1, 1, 1, 1] = 10.0
    time_step!(model, 1.0)
    ...
    isotropic = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=source)
    set!(isotropic, N=1.0)
...
@testset "Directional diffusion source" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=diffusion, timestepper=:ForwardEuler)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=diffusion, timestepper=:SemiImplicitEuler)
    set!(model, N=(x, y, kx, ky) -> ky > 0 && abs(kx) < 1e-12 ? 10.0 : 1.0)
    time_step!(model, 0.1)
    @test all(interior(model.action) .>= 0)
    ...
    bad_model = SpectralWaveModel(grid, cartesian; horizontal_advection=nothing, physics=diffusion)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    negative = DirectionalDiffusion(rate=-0.1)
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=negative)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Directional advection source" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=advection, timestepper=:ForwardEuler)
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=clockwise, timestepper=:ForwardEuler)
    set!(model, N=(x, y, kx, ky) -> ky < 0 && abs(kx) < 1e-12 ? 10.0 : 1.0)
    compute_tendencies!(model)
    weighted_tendency = sum(model.tendencies[1, 1, 1, n] * spectral_weight(cgrid, 1, n)
                            for n in eachindex(cgrid.φ))
    ...
    model = SpectralWaveModel(grid, fgrid; horizontal_advection=nothing, physics=DirectionalAdvection(velocity=0.02))
    ...
    model = SpectralWaveModel(grid, one_direction; horizontal_advection=nothing, physics=advection)
    set!(model, N=1.0)
    compute_tendencies!(model)
    @test maximum(abs, interior(model.tendencies)) == 0

    cartesian = CartesianWaveVectorGrid(Float64; kx=[0.0], ky=[0.0])
    bad_model = SpectralWaveModel(grid, cartesian; horizontal_advection=nothing, physics=advection)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
...
@testset "Radial diffusion source" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=RadialDiffusion(rate=0.05), timestepper=:SemiImplicitEuler)
    set!(model, N=(x, y, kx, ky) -> abs(hypot(kx, ky) - 1.0) < 1e-12 ? 10.0 : 1.0)
    time_step!(model, 0.1)
    @test all(interior(model.action) .>= 0)
...
@testset "Radial advection source" begin
    ...
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=downshift, timestepper=:ForwardEuler)
    ...
    model = SpectralWaveModel(grid, fgrid; horizontal_advection=nothing, physics=RadialAdvection(velocity=0.01))
...
@testset "Nonlinear spectral transfer source" begin
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=negative_rate)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Triad spectral transfer source" begin
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=no_advection, physics=nonresonant)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=no_advection, physics=invalid)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
    ...
    bad_model = SpectralWaveModel(grid, cgrid; horizontal_advection=no_advection, physics=negative_rate)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)

    polar = PolarWaveVectorGrid(; κ=[1.0, 2.0], φ=[0.0])
    bad_model = SpectralWaveModel(grid, polar; horizontal_advection=nothing, physics=triad)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Discrete interaction approximation source" begin
    ...
    bad_model = SpectralWaveModel(grid, polar; horizontal_advection=nothing, physics=dia)
    set!(bad_model, N=1.0)
    @test_throws ArgumentError compute_tendencies!(bad_model)
end
...
@testset "Source positive/damping split" begin
    ...
    for source in sources
        ...
        model = SpectralWaveModel(grid, spectral_grid; horizontal_advection=nothing, physics=source, timestepper=:ForwardEuler)
        set!(model, N=1.25)
...
@testset "Semi-implicit source update" begin
    ...
    damping = BottomFriction(rate=100.0)
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=damping, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
    time_step!(model, 1.0)
    @test model.action[1, 1, 1, 1] ≈ 1 / 101

    relaxation = RelaxationToSpectrum((x, y, kx, ky) -> 2.0; timescale=1.0)
    model = SpectralWaveModel(grid, cgrid; horizontal_advection=nothing, physics=relaxation, timestepper=:SemiImplicitEuler)
    set!(model, N=0.0)
    time_step!(model, 0.5)
    ...
    model = SpectralWaveModel(grid, pgrid; horizontal_advection=nothing, physics=whitecapping, timestepper=:SemiImplicitEuler)
    set!(model, N=1.0)
